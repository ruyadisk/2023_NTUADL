...
from utils import get_prompt, get_bnb_config, get_prompt_fewshot
import argparse
import random
...
def perplexity_fewshot(model, tokenizer, data, max_length=2048):
    
    samples = random.sample(data, 10)

    data_without_samples = [item for item in data if item not in samples]
    instructions = [get_prompt_fewshot(x["instruction"], samples) for x in data_without_samples]
    outputs = [x["output"] for x in data_without_samples]
    data_size = len(data_without_samples)
    # Tokenize data
    tokenized_instructions = tokenizer(instructions, add_special_tokens=False)
    tokenized_outputs = tokenizer(outputs, add_special_tokens=False)
    output_masks = []
    ...
    for i in tqdm(range(data_size)):
        ...
        ppls += perplexity_batch.tolist()
    return {"samples": samples, "perplexities": ppls, "mean_perplexity": np.mean(ppls)}
...
if __name__ == "__main__":
    ...
    # model.eval()
    # for cnt in range(5):
    #     ppl = perplexity_fewshot(model, tokenizer, data)
    #     print(f"Few-Shot Inference #{cnt} results: ")
    #     print("Samples: ", ppl["samples"])
    #     print("Mean perplexity:", ppl["mean_perplexity"])
    ...
    model.eval()
    ppl = perplexity(model, tokenizer, data)
    print("Qlora-finetuned Inference results: ")
    print("Mean perplexity:", ppl["mean_perplexity"])
